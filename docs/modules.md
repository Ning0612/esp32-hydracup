...
HydraCup 共有 21 個模組，位於 `lib/` 目錄下。每個模組對應一個 `.h` 標頭與 `.cpp` 實作；
其中 `Hal`、`HttpAuth`、`HttpServerSupport` 是 ESP-IDF runtime 與 HTTP 安全邏輯的共用基礎模組。
...
| `fsOk` | bool | false | `/webfs` LittleFS mount 可用 |
| `logFsOk` | bool | false | `/logfs` LittleFS mount 可用 |
...
| `ipAddress` | `std::string` | `""` | 目前 IP 位址 |

---

## ConfigManager

**職責**：以 ESP-IDF NVS API 持久化 `AppConfig` 的所有欄位。命名空間：`water_config`。
...
| `saveWifi(const std::string& ssid, const std::string& password)` | 僅更新 WiFi 憑證 |
| `clear()` | 清除整個命名空間 |

NVS key 對應見 [data-formats.md](data-formats.md#nvs-schema)。

所有 NVS 操作都使用 `StorageLock` 共用 mutex，避免多個 worker 同時進入 NVS。
...
**職責**：提供 ESP-IDF runtime 共用的時間與等待 wrapper，避免各模組重複處理底層 API。

**位置**：`lib/Hal/`、`include/hal_*.h`

| 函式 | 實作 |
|------|------|
| `hal_micros()` / `hal_millis()` | 由 ESP-IDF `esp_timer_get_time()` 取得單調時間 |
| `hal_delay_ms(uint32_t milliseconds)` | 以 FreeRTOS `vTaskDelay()` 非阻塞等待 |
| `LOG_INFO` / `LOG_WARN` / `LOG_ERROR` / `LOG_DEBUG` | 對應 ESP-IDF `ESP_LOG*` |

---

## HttpAuth

**職責**：提供登入 session 所需的隨機 token、Cookie 解析、constant-time 比對與
PBKDF2-HMAC-SHA256 密碼雜湊／驗證。

**位置**：`lib/HttpAuth/`、`include/hydracup_auth.h`

密碼只以雜湊寫入 NVS；session token 與 CSRF token 僅存在 runtime memory。

---

## HttpServerSupport

**職責**：封裝 ESP-IDF `esp_http_server` 共用的 request body/header/IP 讀取、JSON／檔案
回應與延遲重啟工具。

**位置**：`lib/HttpServerSupport/`、`include/http_server_support.h`
...
| 方法 | 說明 |
|------|------|
| `begin()` | 初始化 ESP-IDF event loop、netif、WiFi driver 與事件 handler |
| `connectSTA(const std::string& ssid, const std::string& password, uint32_t timeoutMs = 10000)` | 嘗試 STA 連線，帶超時（ms）；回傳是否成功 |
| `startAP(const std::string& ssid, const std::string& password)` | 啟動 SoftAP；回傳是否成功 |
| `loop()` | 由 `hydracup_service` task 呼叫；STA 斷線時觸發重連 |
...
| 方法 | 說明 |
|------|------|
| `begin(ScaleManager&, ConfigManager&, AppState&, AppConfig&, ...)` | 啟動 ESP-IDF HTTP server 並掛載所有路由 |
| `loop()` | 保留的空實作；request 由 `esp_http_server` task callback 處理 |
...
**職責**：framework-independent 的 6 態飲水偵測核心。輸入重量、穩定狀態與時間，
...
| `update()` | 由 `hydracup_control` 呼叫，推進狀態機 |
...
| `setCounterPersistence(DrinkCounterPersistence* store)` | 替換每日計數器持久化 port；預設使用 ESP-IDF NVS counter store |
...
使用的非阻塞 persistence barrier。Restore worker 只讀取 POD，最後由 `hydracup_control` 套用。
...
| 方法 | 說明 |
|------|------|
| `init()` | 初始化 ESP-IDF I2C 與 SSD1306；回傳是否成功 |
| `showBootScreen()` | 顯示開機畫面 |
| `showError(const char* msg)` | 顯示錯誤訊息 |
| `showWifiConnecting(const std::string& ssid)` | 顯示 WiFi 連線中畫面 |
| `showAPMode(const std::string& apSsid, const std::string& apPassword, const std::string& ip)` | 顯示 AP Mode 資訊 |
| `showNormalMode(float weightG, bool stable, float todayMl, uint32_t goalMl, uint32_t drinkCount, float lastDrinkMl, uint32_t nextRemSec, bool wifiOk, const std::string& ip, bool ntpSynced)` | 更新 Normal Mode 顯示資料 |
| `wake()` | 喚醒螢幕（重置睡眠計時器） |
| `sleep()` | 立即關閉螢幕 |
| `update()` | 由 `hydracup_control` 呼叫，驅動頁面切換與睡眠邏輯 |
...
| `notifyOnline(const std::string& ipAddress)` | 裝置上線通知（含 IP） |
| `notifyDrink(float amountMl, float totalMl, uint32_t drinkCount)` | 飲水事件通知（含飲水量、今日總量、次數） |
| `notifyDailySummary(float totalMl, uint32_t drinkCount, const std::string& dateStr)` | 每日摘要通知（回傳是否排程成功） |
| `update()` | 空實作；通知由持久 FreeRTOS worker 處理 |
...
**職責**：使用 ESP-IDF `esp_mqtt_client` 發布目前飲水進度、heartbeat 與事件狀態。
MQTT client 由 ESP-IDF 負責重連；payload 先進入固定大小 queue，再由持久 FreeRTOS worker
呼叫 publish，避免控制 task 直接執行網路操作。

**位置**：`lib/MqttPublisher/`

| 方法 | 說明 |
|------|------|
| `init(AppState& state, const AppConfig& cfg)` | 依設定建立 ESP-IDF MQTT client 與 publish worker |
| `setTimeManager(TimeManager* tm)` | 注入時間來源，讓 payload 可帶 device time |
| `setDailyGoal(uint32_t dailyGoalMl)` | 更新 MQTT payload 使用的每日目標 |
| `loop(float todayTotalMl)` | 由 `hydracup_service` task 呼叫，依 heartbeat 間隔排程狀態發布 |
| `publishStatus(float currentMl, const char* event)` | 將狀態 payload 送入 publish queue |

主要 topic 為 `hydracup/status`；availability 使用 `hydracup/availability`，並以 retained
訊息發布 online／offline 狀態。
...
| 方法 | 說明 |
|------|------|
| `init(bool fsOk)` | 初始化 logfs 狀態、檔案 mutex、queue 與 worker |
| `logDrink(const std::string& ts, float amountMl, float totalMl, TimeManager* tm)` | 將飲水記錄送入非同步 log queue |
...
**職責**：透過 ESP-IDF `esp_sntp_*` API 進行 NTP 時間同步與時戳產生，每 12 小時重新同步
防止漂移。未同步時以 `hal_millis()` 回退。
...
| `update()` | 由 `hydracup_control` 呼叫，以非阻塞 settlement stages 推進每日摘要 |
