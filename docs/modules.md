...
HydraCup 共有 14 個模組，位於 `lib/` 目錄下。每個模組對應一個 `.h` 標頭與 `.cpp` 實作。

---

## AppState

**職責**：持有所有執行時共用狀態的結構體（`AppState`）。以引用方式傳遞給其他模組，作為模組間的通訊橋樑。
...
| `setConfigManager(ConfigManager* mgr)` | 注入 ConfigManager（自動儲存校正值） |
...
**職責**：以 FreeRTOS 獨立任務非同步發送 HTTPS POST 至 Discord Webhook，不阻塞 loop()。
