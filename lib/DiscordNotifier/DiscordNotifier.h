...
public:
    ...
    void notifyOnline(const std::string& ipAddress);
    void notifyDrink(float amountMl, float totalMl, uint32_t drinkCount);
    bool notifyDailySummary(float totalMl, uint32_t drinkCount, const std::string& dateStr);
    void update() {}
    bool isWorkerReady() const { return _workerReady.load(); }
    uint32_t getDroppedCount() const { return _droppedCount.load(); }

private:
    struct TaskParam { char webhookUrl[512]; char body[768]; };
